...
#------------Загрузка графа------------------
import json
myfile = open("E:\Работа\DAO-IT\Данные.txt", mode='r')
G = json.load(myfile)

print(type(G))
print(str(G.keys()))
N = len(G)
#----------Зададим начальную и конечную точку--------------
...
while True:
    ...
    Finish_Point = input()
    if Finish_Point in G.keys():
        break
print("Конечная точка маршрута назначена")
print('\nНачальная точка маршрута: ' + str(Start_point) + ' конечная точка маршрута: ' + str(Finish_Point))

#----------------Получим значение которое будем присваивать необойденным вершинам---------------
        ...
        if N <= len(t):  # Условие выхода из функции
            ...
            while True:
                if b[e] == -1:  # значение ключа (-1) имеет начальная вершина
                    ...
                    print("Общий вес маршрута составил: " + str(p[Finish_Point]))
